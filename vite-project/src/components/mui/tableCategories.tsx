...
interface Props {
  ...
  setFilterType: Dispatch<
    React.SetStateAction<"brand" | "price" | "rating" | "discountPercentage">
  >;
}
        ...
        <TableBody>
          {rows.map((row) => (
            <StyledTableRow key={row?.brand}>
              <StyledTableCell component="th" scope="row" width={"313px"}>
                <span
                  className="hover-text"
                  onClick={() => {
                    setFilter(row?.brand), setFilterType("brand");
                  }}
                >
                  {row?.brand}
                </span>
              ...
              <StyledTableCell component="th" scope="row">
                <span
                  className="hover-text"
                  onClick={() => {
                    setFilter(row?.price), setFilterType("price");
                  }}
                >
                  {row?.price}
                </span>
              ...
              <StyledTableCell align="right">
                <span
                  className="hover-text"
                  onClick={() => {
                    setFilter(row?.rating), setFilterType("rating");
                  }}
                >
                  {row?.rating}
                </span>
              ...
              <StyledTableCell align="right">
                <span
                  className="hover-text"
                  onClick={() => {
                    setFilter(row?.discountPercentage),
                      setFilterType("discountPercentage");
                  }}
                >
